...
bool V3D_AddVertexesToScene (RENDER_STRUCT* scene)
{
	...
	//	emit_float(&p, 0.0f);											// Varying 2 (Red)
		...
		printf("Render piss2:%x \n",p);
		printf("Render qoc2:%x \n",q);
...
bool V3D_SetupRenderControl (RENDER_STRUCT* scene, VC4_ADDR renderBufferAddr)
{
	if (scene)
	{
		...
		*/
		...
		emit_uint16_t(&p, renderBufferAddr && 0x0000FFFF);
		emit_uint16_t(&p, renderBufferAddr>>16);
		...
		for (int x = 0; x < scene->binWth; x++) {
			for (int y = 0; y < scene->binHt; y++) {
				...
				scene->banana[0] = scene->tileDataBufferVC4;
				scene->banana[1] = scene->tileDataBufferVC4 + (y * scene->binWth + x) * 32;
